 ...
 *
 * Reachability, not just visibility: withAdminAudit runs requireAdmin() before
 * this handler is entered, and it redirects rather than returning — a non-admin
 * cannot reach the bot fill by guessing the URL, and the UI that offers it
 * lives under /admin, which is gated by its own layout. Hiding a button is not
 * a permission model.
 *
...
export const POST = withAdminAudit<{ id: string }>(
  { action: "tournament.fill-bots", targetType: "tournament", targetIdParam: "id" },
  async ({ req, params, audit }) => {
    ...
    // Recorded before anything is read or seated, so the refusals below — the
    // wrong format, the wrong status, and above all the unacknowledged LIVE
    // override — are logged as attempts with the terms they were attempted on.
    // "Who kept trying to seat bots in a real event" is precisely the pattern
    // this trail exists to make visible.
    audit.meta({ acknowledgeLive: acknowledgeLive === true, limit: limit ?? null });
    ...
    // Taken from the row already in hand rather than through
    // auditTournamentLabel, which would be a second read of the same record.
    audit.label(t.name);
    audit.meta({ isTest: t.isTest, entryTetri: t.entryTetri });
    ...
    try {
      ...
      // How many bots were really seated and how much was really minted from
      // treasury to pay their entry fees. Bot money is minted and swept, so an
      // operator reconciling treasury later needs the fill that created it named
      // here rather than inferred from a pile of entry postings.
      audit.meta({
        seated: result.seated,
        mintedTetri,
        mode: result.mode,
        alreadyFull: result.alreadyFull,
      });
